export class FormHandler {
  ...
  sanitizeInput = (field) => {
    field.value = field.value.replace(/((\<)(.*)(\>))+/gi, '&lt;$3&gt;');
    return field.value;
  };
  ...
  showModal() {
    ...
    this.fields.map((field) => {
      ...
      if (formField) {
        modal.innerHTML += `<p>${
          formField.value ? field.desc + ' ' + this.sanitizeInput(formField) :
          field.ifEmpty}</p>`;
      }
    });
...
const overlayStyles =
  'position: fixed;top: 0;left: 0;width: 100%;height: 100%;z-index: 10;background-color: rgba(0,0,0,0.5);';
const modalStyles =
  'position: absolute; top: 50%; left: 50%; transform: translate(-50%,-50%); background: #fff; width: 300px; max-height: 380px; padding: 25px;';
