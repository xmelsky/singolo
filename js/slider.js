...
class Slider {
  ...
  setActive() {
    this.images.map((e, i) => {
      if (e.classList.contains("phone-screen__image-verical") && e.classList.contains("active")) this.index = i;
    });
  }

  removeAllClases = ({ target }) => {
    this.images.map((e) => {
      e.classList.remove("move_right", "move_left", "move_from_right", "move_from_left");
    });
